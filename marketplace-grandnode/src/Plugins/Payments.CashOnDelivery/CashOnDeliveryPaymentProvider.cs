...
public class CashOnDeliveryPaymentProvider : IPaymentProvider
{
    ...
    public string FriendlyName
    {
        get
        {
            var translated = _translationService.GetResource(CashOnDeliveryPaymentDefaults.FriendlyName);
            return translated == CashOnDeliveryPaymentDefaults.FriendlyName
    ...
    {
        // BEH confirms the order and sends payment instructions after checkout.
        // No bank details or payment form should be requested on the storefront.
        return await Task.FromResult(true);
    }

    public async Task<string> Description()
    {
        var resourceKey = "Plugins.Payment.CashOnDelivery.PaymentMethodDescription";
        var translated = _translationService.GetResource(resourceKey);
        return await Task.FromResult(translated == resourceKey
            ? "Tu pedido queda pendiente mientras BEH confirma disponibilidad, logística y condiciones de pago."
            : translated);
    }
