...
public class PageHeadBuilder : IPageHeadBuilder
{
    ...
    {
        string result;
        var defaultTitle = Environment.GetEnvironmentVariable("BEH_DEFAULT_TITLE") ?? _seoSettings.DefaultTitle;
        ...
        if (!string.IsNullOrEmpty(titleParts))
        {
            if (addDefaultTitle)
                result = _seoSettings.PageTitleSeoAdjustment
                    ? string.Join(_seoSettings.PageTitleSeparator, defaultTitle, titleParts)
                    : string.Join(_seoSettings.PageTitleSeparator, titleParts, defaultTitle);
            else
                result = titleParts;
        }
        else
